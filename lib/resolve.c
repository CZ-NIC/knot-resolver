...
/** Create empty SERVFAIL answer with EDNS. */
...
static void answer_finalize(struct kr_request *request)
{
	...
	for (knot_section_t sect = KNOT_ANSWER; sect <= KNOT_ADDITIONAL; ++sect) {
		...
		const int ret = knot_pkt_begin(answer, sect)
			|| write_extra_ranked_records(sel, reorder, answer, &secure,
...
{
	/* In case of failure, we don't put the last query's records into answer,
