				      ...
				      size_t *choice_index)
{
	...
	struct kr_transport *transport = mm_calloc(mempool, 1, sizeof(struct kr_transport));
...
void kr_server_selection_init(struct kr_query *qry)
{
	...
	struct local_state *local_state = mm_calloc(mempool, 1, sizeof(struct local_state));
