...
/* Getting current stack trace as string. */

/**
 * Fill char_array with one-line string in the form:
...
#define kr_log_get_shorttrace(char_array) { \
	size_t len = snprintf(char_array, sizeof(char_array), "%s:%d <- ", __FUNCTION__, __LINE__); \
	kr_log_get_trace(char_array + len, sizeof(char_array) - len, 2, 30, " <- "); \
}

/**
 * Fill buf with textual representaion of the current call stack from bottom to upper levels
 * with levels separated by sep.
 * The first_level may be 1 to skip just this function call, or more to skip higher ones.
 * The number of returned levels is limited by both last_level and buf_size
 * and no more levels are printed after the first one outside kresd, libkres and libknot.
 *
