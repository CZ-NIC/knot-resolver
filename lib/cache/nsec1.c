...
success:
	...
	kr_cache_kru_access(cache->kru, key_nsec.data, key_nsec.len, "leq_nsec1");  // hits only
	return NULL;
}
		    ...
		    const struct kr_query *qry, struct kr_cache *cache)
{
	...
	if (cmp == 1) {
		...
		uint32_t new_ttl;
		kr_cache_kru_access(cache->kru, key.data, key.len, "nsec1_src_synth"); // TODO remove, probably redundant, hit (exact/cover) or miss
		const char *err = find_leq_NSEC1(cache, qry, key, k, &val,
