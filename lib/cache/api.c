		...
		uint8_t rank, trie_t *nsec_pmap, knot_mm_t *pool, bool *needs_pkt)
{
	...
	if (kr_fails_assert(entry_h_consistent_E(val_new_entry, rr->type)))
		return kr_error(EINVAL);
	kr_cache_kru_access(cache->kru, key.data, key.len, "stash_rrset");
			...
			const struct kr_query *qry/*logging*/)
{
	...
	kr_cache_kru_access(cache->kru, key.data, key.len, "stash_nsec_p");
	if (log_refresh_by) {
			...
			struct kr_cache_p *peek)
{
	...
	*peek = (struct kr_cache_p){
	...
	kr_cache_kru_access(cache->kru, key.data, key.len, "peek_exact_real"); // hits only
	return kr_ok();
}
