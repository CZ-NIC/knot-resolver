...
int peek_nosync(kr_layer_t *ctx, knot_pkt_t *pkt)
{
	 ...
	 **** 1a. exact name+type match (can be negative, mainly in insecure zones) */
	{
		...
		if (!ret) {
			kr_cache_kru_access(cache->kru, key.data, key.len, "peek_nosync:exact");  // hits only
	...
	if (ans.rcode != PKT_NOERROR) {
		...
		if (ret) return ctx->state;
		kr_cache_kru_access(cache->kru, key.data, key.len, "peek_nosync:SOA");  // hits only
	}
		    ...
		    const struct kr_query *qry, struct kr_cache *cache)
{
	...
	kr_cache_kru_access(cache->kru, key.data, key.len, "try_wild"); // hits only
	return kr_ok();
}
...
success:
	k->zlf_len = zlf_len;
	kr_cache_kru_access(cache->kru, key.data, key.len, "closest_NS"); // hits only
	return kr_ok();
}
