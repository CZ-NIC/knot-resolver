...
#include "lib/cache/kru.h"
#include "lib/utils.h"

#define kr_cache_kru_access_cdb(...) { if (env->is_cache) kr_cache_kru_access_cdb(__VA_ARGS__); } // TODO remove
...
struct lmdb_env
{
	...
	struct kr_cache_kru *kru; // TODO remove
		...
		const knot_db_val_t *key, knot_db_val_t *val, int maxcount)
{
	...
	for (int i = 0; i < maxcount; ++i) {
		...
		stats->read++;
		kr_cache_kru_access_cdb(env->kru, _key.mv_data, _key.mv_size, "readv");
			...
			knot_db_val_t *val, struct kr_cdb_stats *stats)
{
	...
	stats->write++;
	kr_cache_kru_access_cdb(env->kru, _key.mv_data, _key.mv_size, "write");
		...
		knot_db_val_t *key, knot_db_val_t keyval[][2], int maxcount)
{
	...
	stats->match++;
	kr_cache_kru_access_cdb(env->kru, cur_key.mv_data, cur_key.mv_size, "match-prefix");
	...
	while (ret == MDB_SUCCESS) {
		...
		/* Add to result set */
		if (results < maxcount) {
			kr_cache_kru_access_cdb(env->kru, cur_key.mv_data, cur_key.mv_size, "matched");
		...
		knot_db_val_t *key, knot_db_val_t *val)
{
	...
	stats->read_leq++;
	kr_cache_kru_access_cdb(env->kru, key2_m.mv_data, key2_m.mv_size, "leq-query");
...
success:
	/* finalize the output */
	kr_cache_kru_access_cdb(env->kru, key2_m.mv_data, key2_m.mv_size, "leq");
		...
		knot_db_val_t *key, knot_db_val_t *val)
{
	...
	MDB_val val2_m = { 0, NULL };
	stats->read_less++;
	kr_cache_kru_access_cdb(env->kru, key2_m.mv_data, key2_m.mv_size, "less-query");
	// It could keep on the same `key` when MDB_PREV was used.
	ret = mdb_cursor_get(curs, &key2_m, &val2_m, MDB_PREV_NODUP);
	if (!ret) {
		/* finalize the output */
		kr_cache_kru_access_cdb(env->kru, key2_m.mv_data, key2_m.mv_size, "less");
		*key = val_mdb2knot(key2_m);
		*val = val_mdb2knot(val2_m);
		return 1;
