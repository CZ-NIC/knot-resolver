...
static int set_mapsize(MDB_env *env, size_t map_size)
{
	...
	return mdb_env_set_mapsize(env, map_size);
}
...
static int reopen(struct lmdb_env *env, struct kr_cdb_stats *stats)
{
	...
	cdb_close_env(env, stats);
	return cdb_open(env, path_copy, mapsize, stats);
}
