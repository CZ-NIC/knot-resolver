...
mp_free_big_chunk(struct mempool_chunk *chunk)
{
	...
	printf("FREE_BIG_CHUNK: size %8zu   %s\n",
			chunk->size, trace);
...
mp_free_reusable_chunk(struct mempool_chunk *chunk) {
	...
	if (reusable) {
		...
		char trace[150]; kr_log_get_shorttrace(trace);
		printf("FREE_REUSABLE: size orig %8zu, new %8zu   %s\n",
...
mp_grow_internal(struct mempool *pool, size_t size)
{
	...
	if (pool->idx) {
		...
		MEMCHECK_UNDEFINED(p, pool->state.free[pool->idx]);
		memcpy(p, ptr, avail);
...
mp_open(struct mempool *pool, void *ptr)
{
	...
	MEMCHECK_UNDEFINED(ptr + size, pool->state.free[pool->idx]);
	pool->state.free[pool->idx] += size;
	return size;
}
