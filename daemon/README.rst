...
The default port can also be overriden by using an empty ``ListenDatagram=`` or
``ListenStream=`` directive. This can be used when you want to configure
kresd to listen on all IPv4/IPv6 network interfaces (use ``0.0.0.0`` instead of
``[::]`` if you've disabled IPv6 support in kernel).
