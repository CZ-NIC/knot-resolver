	...
	size_t datalen, void *user_data, void *stream_user_data)
{
	...
	struct pl_quic_conn_sess_data *qconn =
...
bool kr_quic_stream_exists(struct pl_quic_conn_sess_data *conn,
		int64_t stream_id)
{
		...
		int64_t stream_id, const uint8_t *data, size_t len, bool fin)
{
	...
	struct kr_quic_stream *stream = kr_quic_conn_get_stream(conn, stream_id, true);
	if (stream == NULL) {
		return KNOT_ENOENT;
	}
