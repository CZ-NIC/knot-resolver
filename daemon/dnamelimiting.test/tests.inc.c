...
#include "daemon/dnamelimiting.c"
#undef kr_now
...
static void test_rrl(void **state) {
	...
	char mmap_file[64];
	stpcpy(stpcpy(mmap_file, tmpdir), "/dnamelimiting");
	dnamelimiting_init(mmap_file, RRL_TABLE_SIZE, RRL_INSTANT_LIMIT, RRL_RATE_LIMIT, 0, 0, false);

	if (KRU.initialize == KRU_GENERIC.initialize) {
		struct kru_generic *kru = (struct kru_generic *) dnamelimiting->kru;
		memset(&kru->hash_key, RRL_SEED_GENERIC, sizeof(kru->hash_key));
	} else if (KRU.initialize == KRU_AVX2.initialize) {
		struct kru_avx2 *kru = (struct kru_avx2 *) dnamelimiting->kru;
