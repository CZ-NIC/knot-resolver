...
-- Helpers for reading message section counts
...
-- Helpers for reading message header flags
...
ffi.metatype( knot_pkt_t, {
	...
	__index = {
		...
		question = function(pkt, qname, qclass, qtype)
			assert(pkt ~= nil)
			assert(const_class_str[qclass], string.format('invalid class: %s', qclass))
			assert(const_type_str[qtype], string.format('invalid type: %s', qtype))
