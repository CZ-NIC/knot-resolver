...
local function test_de_serialization_autodesc(orig_val, desc)
...
local function gen_test_tables_supported(level)
	...
	for i=1, math.random(0, max_items_per_table) do
		local val_as_table = (level <= maxlevel) and math.random() < 0.1
		local key, val
