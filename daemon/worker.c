...
ssize_t worker_gnutls_push(gnutls_transport_ptr_t h, const void *buf, size_t len)
{
	...
	struct session *session = t->session;
	assert(session);
	...
	ssize_t ret = -1;
	int res = uv_write(write_req, (uv_stream_t *)handle, uv_buf, 1, write_cb);
	if (res == 0) {
		if (task) {
			qr_task_ref(task); /* Pending ioreq on current task */
			struct request_ctx *ctx = task->ctx;
			struct session *source_session = ctx ? ctx->source.session : NULL;
			if (source_session && handle != session_get_handle(source_session)) {
				struct sockaddr *peer = session_get_peer(source_session);
				worker->stats.tls += 1;
