					  ...
					  uint32_t uid)
{
	...
	if (session) {
		...
		memcpy(&ctx->peer.ip, peer, kr_sockaddr_len(peer));
		req->qsource.addr = &ctx->peer.ip;
	}
...
static int request_start(struct request_ctx *ctx, knot_pkt_t *query)
{
	...
	/* session can be empty if request was generated by kresd itself */
...
static int qr_task_register(struct qr_task *task, struct session *session)
{
	...
	struct request_ctx *ctx = task->ctx;
	assert(ctx && (ctx->session == NULL || ctx->session == session));
	ctx->session = session;
...
static int qr_task_finalize(struct qr_task *task, int state)
{
	...
	struct request_ctx *ctx = task->ctx;
	struct session *source_session = ctx->session;
	...
	if (src_handle->type != UV_UDP && src_handle->type != UV_TCP) {
		...
		ret = qr_task_send(task, source_session, &ctx->peer.ip, ctx->req.answer);
	}
	...
	if (ret != kr_ok()) {
		...
		while (!session_tasklist_is_empty(source_session)) {
			...
			assert(c->session == source_session);
			c->session = NULL;
...
int worker_end_tcp(struct session *session)
{
	...
	while (!session_waitinglist_is_empty(session)) {
		...
		if (session_flags(session)->outgoing) {
			...
			assert(task->ctx->session == session);
			task->ctx->session = NULL;
		}
