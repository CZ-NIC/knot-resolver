...
int defer_init(const char *mmap_file, uint32_t log_period, int cpus)
{
	...
	if (ret == MMAPPED_WAS_FIRST) {
		kr_log_info(SYSTEM, "Initializing defer...\n");
		...
		kr_log_info(SYSTEM, "Defer initialized (%s).\n", (defer->using_avx2 ? "AVX2" : "generic"));
		...
		defer = defer_mmapped.mem;
		kr_log_info(SYSTEM, "Using existing defer data (%s).\n", (defer->using_avx2 ? "AVX2" : "generic"));
	} else goto fail;
...
fail:
	...
	kr_log_crit(SYSTEM, "Initialization of shared defer data failed.\n");
	return ret;
}
