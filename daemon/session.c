...
static void waitinglist_clear_source(struct session *session, struct session *source)
{
	typedef queue_it_t(struct qr_task *) it_t;
	for (it_t it = queue_it_begin(session->waiting); !queue_it_finished(it); queue_it_next(it)) {
		struct qr_task *task = queue_it_val(it);
		struct request_ctx *ctx = worker_task_get_request(task);
		if (worker_request_get_source_session(ctx) == source) {
			worker_request_set_source_session(ctx, NULL);
		}
	}
}

static void tasklist_clear_source(struct session *session, struct session *source)
{
...
static int clear_source_session(const char *key, void *val, void *baton)
{
	struct session *source_session = baton;
	struct session *session = (struct session *)val;
	tasklist_clear_source(session, source_session);
	waitinglist_clear_source(session, source_session);
	return 0;
}
...
void session_close(struct session *session)
{
	...
	if (session->peer.ip.sa_family != AF_UNSPEC) {
		...
		if (handle->type== UV_TCP && !session->sflags.outgoing) {
			map_walk_prefixed(&worker->tcp_connected, "", clear_source_session, session);
		}
...
int session_tasklist_finalize_expired(struct session *session)
{
	...
	for (it = trie_it_begin(t); !trie_it_finished(it); trie_it_next(it)) {
		trie_val_t *v = trie_it_val(it);
		struct qr_task *t = (struct qr_task *)*v;
		if ((now - worker_task_creation_time(t)) >= KR_RESOLVE_TIME_LIMIT) {
			queue_push(q, t);
		}
	}
	trie_it_free(it);
