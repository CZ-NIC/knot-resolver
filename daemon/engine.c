...
static int l_engine_ipc_cmd(lua_State *L, int fd, const char *cmd, size_t *out_len)
{
	...
	do {
		auto_free char *cmd_json = malloc(strlen(cmd) + strlen("tojson()") + 1);
		sprintf(cmd_json, "tojson(%s)", cmd);

		expr_checked(write(fd, cmd_json, strlen(cmd_json)) == strlen(cmd_json));
		sync();
...
static int l_map(lua_State *L)
{
	...
	auto_free char *cdir = malloc(strlen(cwd) + strlen("/control/") + 1);
	sprintf(cdir, "%s/control/", cwd);

	DIR *dr = opendir(cdir);
	if (dr == NULL)
		lua_error_p(L, "Control sockets directory doesn't exists (%s)", cdir);
