...
if not has_doh2 then
	if string.find(err, 'compiled without libnghttp2') ~= nil then
		pass('skipping doh2 tests (compiled without libnghttp2)')
		done()
	else
		assert(false, 'failed to bind "doh2" socket kind')
	end
else
