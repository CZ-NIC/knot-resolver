...
if not worker.bg_worker then
	...
	-- test whether running commands works
	local function test_worker_map()
		local status, res = pcall(worker.map, 'worker.info()')
		ok(status, 'worker.map works')
		same(type(res), 'table', 'worker.map returns a table')
		same(#res, worker.count, 'worker.map returns one result for each fork')
		same(type(res[1]), 'table', 'worker.map of worker.info returns tables')
	end
