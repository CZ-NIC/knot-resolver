...
category_t kr_gc_categorize(struct kr_cache_top *top, struct kr_gc_cat_record_info *info)
{
...
void kr_gc_cat_analyze(struct kr_cache_top *top, struct kr_gc_cat_record_info *info, void *analysis_ctx)
{
	...
	category_t cat = kr_gc_categorize(top, info);
		...
		uint8_t remove_perc, uint8_t unsched_perc)
{
	...
	struct { size_t
		...
		P_keep_ekeydata, P_remove_ekeydata_valid_keep, P_remove_ekeydata_valid_remove, P_remove_ekeydata_exp,

		// sums
		P_total, S_total, E_total_valid, E_total_exp,
		P_total_ekeydata_valid,
		remove, total
	} sizes = { 0 };
	...
	for (; (size <= amount_tofree) && (c > 0); c--) {
		...
		sizes.P_remove                       += analysis->categories_sizes_P[c];
		sizes.P_remove_ekeydata_valid_remove += analysis->categories_sizes_P_ekeydata[c];
	}
	...
	for (; (size <= amount_tounsched) && (c > 0); c--) {
		...
		sizes.P_remove                       += analysis->categories_sizes_P[c];
		sizes.P_remove_ekeydata_valid_keep   += analysis->categories_sizes_P_ekeydata[c];
	}
	...
	for (; c > 0; c--) {
		...
		sizes.P_keep_ekeydata                += analysis->categories_sizes_P_ekeydata[c];
	}

	// sums
	sizes.P_total_ekeydata_valid =
		sizes.P_keep_ekeydata +
		sizes.P_remove_ekeydata_valid_keep +
		sizes.P_remove_ekeydata_valid_remove;
	sizes.E_total_valid =
		sizes.E_keep_valid +
	...
	printf("Canceling prefetch for\n"
			"  %8.2f MB of kept valid RRs (limit category %d),\n"
			"  %8.2f MB of expired RRs (not updated in time, incl. those for removal);\n"
			"removing (limit category %d)\n"
			"  %8.2f MB / %8.2f MB of valid RRs with scheduled prefetch,\n"
			...
			sizes.P_remove_ekeydata_valid_keep / MB, summary->limit_category_unsched,
			sizes.P_remove_ekeydata_exp / MB,
			summary->limit_category_remove,
			sizes.P_remove_ekeydata_valid_remove / MB, sizes.P_total_ekeydata_valid / MB,
			(int64_t)(sizes.E_remove_valid - sizes.P_remove_ekeydata_valid_remove) / MB,    // may be a little negative due to some imprecisions
				(int64_t)(sizes.E_total_valid - sizes.P_total_ekeydata_valid) / MB,           // here too
...
{
	const category_t cat = kr_gc_categorize(top, info);
