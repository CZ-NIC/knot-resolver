...
local function rpz_parse(action, path)
	...
	while true do
		...
		if errstr then
			log_info(ffi.C.POLICY, 'RPZ %s:%d: %s', path, tonumber(parser.line_counter), errstr)
		end
		...
		if prefix_labels < 0 then
			log_info(ffi.C.POLICY, 'RPZ %s:%d: RR owner "%s" outside the zone (ignored)',
		...
		if parser.r_type == kres.type.CNAME then
			if action_map[rdata] then
				...
				log_info(ffi.C.POLICY, 'RPZ %s:%d: CNAME with custom target in RPZ is not supported yet (ignored)',
			...
			if #name then
				...
				if is_bad == true or (is_bad == false and prefix_labels ~= 0) then
					log_warn(ffi.C.POLICY, 'RPZ %s:%d warning: RR type %s is not allowed in RPZ (ignored)',
				...
				elseif is_bad == nil then
					...
					if act == nil then
						...
						if parser.r_ttl ~= act.ttl then -- be conservative
							log_warn(ffi.C.POLICY, 'RPZ %s:%d warning: different TTLs in a set (minimum taken)',
								path, tonumber(parser.line_counter))
...
function policy.rpz(action, path, watch)
	...
	if watch ~= false then
			...
			worker.coroutine(function ()
				for _, name in watcher:changes() do
					...
					if name == file then
						...
						log_info(ffi.C.POLICY, 'RPZ reloading: ' .. name)
		...
		elseif watch then -- explicitly requested and failed
			...
			log_info(ffi.C.POLICY, 'lua-cqueues required to watch and reload RPZ file, continuing without watching')
		end
	end
...
local debug_logline_cb = ffi.cast('trace_log_f', function (_, msg)
	...
	log_fmt(ffi.C.LOG_GRP_POLICY, LOG_DEBUG, "%s", msg)
end)
