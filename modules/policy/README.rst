...
Filters
-------
A *filter* selects which queries will be affected by specified Actions_. Several policy filters are available:
...
.. note:: Due to performance reasons, this filter requires domain names in DNS wire format, not their textual representation. Each label in the name must be prefixed with its length. Always use convenience function :func:`policy.todnames` for automatic conversion from strings! For example:
...
.. function:: rpz(default_action, path, [watch])
   ...
   Implements a subset of `Response Policy Zone` (RPZ_) stored in zonefile format. See :ref:`response-policy-zones` below for details.
...
.. function:: custom_filter(state, query)
   ...
   :return: An :ref:`action <mod-policy-actions>` function or ``nil`` if filter did not match.
...
An *action* is a function which modifies DNS request, and is either of type *chain* or *non-chain*:
  ...
  * :ref:`non-chain-actions` modify state of the request and stop rule processing. An example of such action is :ref:`forwarding`.
  * :ref:`chain-actions` modify state of the request and allow other rules to evaluate and act on the same request. One such example is :func:`policy.MIRROR`.
...
.. function:: custom_action(state, request)
   ...
   :return: Returning a new :c:type:`kr_layer_state` prevents evaluating other policy rules. Returning ``nil`` creates a :ref:`chain action <chain-actions>` and allows to continue evaluating other rules.
