...
These rules can be overriden by action :func:`policy.PASS`, see `Policy examples`_ below.  For debugging purposes you can also add ``modules.unload('policy')`` to your config to unload the module.
...
.. py:attribute:: policy.QTRACE
   ...
   Pretty-print DNS response packets into the verbose log for the query and its sub-queries.  It's useful for debugging weird DNS servers.
...
.. function:: FORWARD(ip_address)
   ...
   A variant which uses encrypted DNS-over-TLS transport is called :func:`policy.TLS_FORWARD`.
...
Such data would typically be rejected by DNSSEC validation starting from the ICANN root keys.  Therefore, if you trust the resolver and your link to it, you can simply use the :func:`policy.STUB` action instead of :func:`policy.FORWARD` to avoid validation only for those subtrees.
