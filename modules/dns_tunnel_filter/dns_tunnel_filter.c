...
static uint8_t qname_to_registrable_dname(const struct kr_request *req, knot_dname_t *out, size_t out_size)
{
	...
	if (knot_dname_from_str(out, rg_fqdn, out_size) == NULL)
		return -1;
	return 0;
}
...
// this logic comes from kr_rule_consume_tags()
	...
	if (req->qsource.addr->sa_family == AF_INET6) {
		...
		for (int i = 0; i < V6_PREFIXES_CNT; ++i) {
			uint64_t price = (req->qsource.price_factor16 * (uint64_t)price_scale_factor
	...
	float tunnel_prob = -1;
	bool req_do_apply = det_do_apply;
	bool req_do_audit = det_do_audit;
	kr_rule_tags_t req_tags = req_do_apply ? det_tags_apply : det_tags_audit;
