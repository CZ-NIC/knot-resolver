...
local function address_callback(pkt, req)
	if pkt == nil or pkt:rcode() ~= kres.rcode.NOERROR then
		...
		log_warn(ffi.C.PRIMING, "cannot resolve address '%s', type: %d", kres.dname2str(pkt:qname()), pkt:qtype())
	else
	...
	if internal.to_resolve == 0 then
		if count_addresses(internal.nsset) == 0 then
			log_warn(ffi.C.PRIMING, "cannot resolve any root server address, next priming query in %d seconds", priming.retry_time / sec)
			internal.event = event.after(priming.retry_time, internal.prime)
		else
			publish_hints(internal.nsset)
			log_info(ffi.C.PRIMING, "triggered priming query, next in %d seconds", internal.min_ttl)
...
local function priming_callback(pkt, req)
	if pkt == nil or pkt:rcode() ~= kres.rcode.NOERROR then
		log_warn(ffi.C.PRIMING, "cannot resolve '.' NS, next priming query in %d seconds", priming.retry_time / sec)
		internal.event = event.after(priming.retry_time, internal.prime)
		return nil
	end
	local section = pkt:rrsets(kres.section.ANSWER)
