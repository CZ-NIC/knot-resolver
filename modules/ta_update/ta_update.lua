...
local function check_upstream(keyset, new_keys)
	...
	for _, rr in ipairs(new_keys) do
		...
		-- TODO: otestovat lepsi podminku (ta and key_revoked)
		if key_revoked then
			-- I see revoked key
			ta_update.cb_unmanagedkey_change()
		end
...
function check_static_key(zname)
	return true
end
...
function ta_update.start(zname, managed)
	...
	if not keyset.managed then
		warn('[ta_update] TA is configured as unmanaged; we recommended remove it and '
