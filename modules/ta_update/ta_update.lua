...
local function ta_find(keyset, rr)
	...
	if rr_tag < 0 or rr_tag > 65535 then
		log_warn(ffi.C.TAUPDATE, string.format('ignoring invalid or unsupported RR: %s: %s',
	...
	for i, ta in ipairs(keyset) do
		...
		if ta_tag < 0 or ta_tag > 65535 then
			log_warn(ffi.C.TAUPDATE, string.format('[ta_update] ignoring invalid or unsupported RR: %s: %s',
...
local function ta_present(keyset, rr, hold_down_time)
	...
	if ta then
		...
		if rr.state ~= key_state.Valid or verbose() then
			log_info(ffi.C.TAUPDATE, 'key: ' .. key_tag .. ' state: '..ta.state)
		end
		return true
...
local function ta_missing(ta, hold_down_time)
	...
	elseif ta.state == key_state.Missing and os.difftime(ta.timer, os.time()) <= 0 then
		ta.state = key_state.Removed
		log_info(ffi.C.TAUPDATE, 'key: '..key_tag..' removed because missing for too long')
		keep_ta = false

	-- Purge pending key
	elseif ta.state == key_state.AddPend then
		log_info(ffi.C.TAUPDATE, 'key: '..key_tag..' purging')
		keep_ta = false
	end
	log_info(ffi.C.TAUPDATE, 'key: '..key_tag..' state: '..ta.state)
	return keep_ta
...
local function update(keyset, new_keys)
	...
	if not trust_anchors.keyset_publish(keyset) then
		...
		log_info(ffi.C.TAUPDATE, 'refreshed trust anchors for domain ' .. kres.dname2str(keyset.owner) .. ' are:\n'
...
local function unmanagedkey_change(file_name)
	log_warn(ffi.C.TAUPDATE, 'you need to update package with trust anchors in "%s" before it breaks', file_name)
end
...
local function active_refresh(keyset, pkt, req, managed)
	...
	if pkt ~= nil and pkt:rcode() == kres.rcode.NOERROR then
		...
		if qry.flags.DNSSEC_BOGUS == true then
			log_warn(ffi.C.TAUPDATE, 'active refresh failed, update your trust anchors in "%s"', keyset.filename)
		elseif pkt == nil then
			log_warn(ffi.C.TAUPDATE, 'active refresh failed, answer was dropped')
		else
			log_warn(ffi.C.TAUPDATE, 'active refresh failed for ' .. kres.dname2str(keyset.owner)
...
local function refresh_plan(keyset, delay, managed)
	...
	track_cfg.event = event.after(delay, function ()
		log_info(ffi.C.TAUPDATE, 'refreshing TA for ' .. owner_str)
		...
		function (pkt, req)
			...
			log_info(ffi.C.TAUPDATE, 'next refresh for ' .. owner_str .. ' in '
