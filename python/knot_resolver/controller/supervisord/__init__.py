...
from knot_resolver.controller.exceptions import KresControllerError, KresControllerExec
...
async def _start_supervisord(config: KresConfig) -> None:
    ...
    if res != 0:
        raise KresControllerError(f"Supervisord exited with exit code {res}")
...
async def _reload_supervisord(config: KresConfig) -> None:
    ...
    try:
        ...
        raise KresControllerError(f"supervisord reload failed: {e}") from e
...
def _list_running_subprocesses(config: KresConfig) -> Dict[SupervisordKresID, SubprocessStatus]:
    try:
        ...
        raise KresControllerError(f"failed to get info from all running processes: {e}") from e
...
class SupervisordSubprocess(Subprocess):
    ...
    def status(self) -> SubprocessStatus:
        try:
            ...
            raise KresControllerError(f"failed to get status from '{self.id}' process: {e}") from e
    ...
    def _start(self) -> None:
        ...
        try:
            ...
            raise KresControllerError(f"failed to start '{self.id}'") from e
