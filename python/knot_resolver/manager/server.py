...
class Server:
    ...
    # ok.
    def __init__(self, store: ConfigStore, config_path: Optional[tuple[Path, ...]], manager: KresManager) -> None:
        ...
        self._config_path: Optional[tuple[Path, ...]] = config_path
...
async def start_server(args: KresArgs) -> int:
    ...
    try:
        ...
        for config_file in args.config:
            ...
            if args.config[0].parent != config_file.parent:
                logger.warning(
                    ...
                    f" from '{args.config[0]}', which is used as the prefix for relative paths."
        ...
        server = Server(config_store, args.config, manager)
