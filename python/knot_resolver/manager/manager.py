...
class _FixCounter:
    ...
    def try_decrease(self) -> None:
        if time.time() - self._timestamp > FIX_COUNTER_DECREASE_INTERVAL_SEC:  # noqa: SIM102
            if self._counter > 0:
                logger.info(
...
class KresManager:  # pylint: disable=too-many-instance-attributes
    ...
    async def _processes_watchdog(self) -> None:  # pylint: disable=too-many-branches  # noqa: C901, PLR0912
        while True:
            ...
            try:
                ...
                for eid in expected_ids:
                    ...
                    if detected_subprocesses[eid] is SubprocessStatus.FATAL:
                        if self._policy_loader and self._policy_loader.id == eid:
                            logger.info(
                                "Subprocess '%s' is skipped by WatchDog because its"
                                " status is monitored in a different way.",
                                eid,
                            )
                            continue
                ...
                invoke_callback = True
                logger.error(f"Processes watchdog failed with KresSubprocessControllerError: {e}")
            except asyncio.CancelledError:
                raise
            except BaseException:
                invoke_callback = True
