...
if KAFKA_LIB:
    ...
    def check_chunk_headers(headers: Headers) -> None:
        ...
        if index and not total:
            KresKafkaClientError("missing 'total-chunks' message header")
        if total and not index:
            KresKafkaClientError("missing 'chunk-index' message header")
        if index and total and index > total:
            KresKafkaClientError(f"'chunk-index' value cannot be bigger than 'total-chunks' value '{index} > {total}'")
        ...
        # received chunk of data
            ...
            for i in range(1, total + 1):
                ...
                if path.exists():
                    ...
                    logger.info(f"The file '{file_name}' cannot be assembled yet: missing chunk {i}")
