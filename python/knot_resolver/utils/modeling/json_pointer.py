...
# JSONPtrAddressable = Optional[
#     Union[Dict[str, "JSONPtrAddressable"], List["JSONPtrAddressable"], int, float, bool, str, None]
# ]
...
class _JSONPtr:
    ...
    def resolve(
        ...
        for token in self.tokens:
            if current is None:
                raise ValueError(
                    "JSON pointer cannot reference nested non-existent object:"
                    f" object at ptr '{current_ptr}' already points to None, cannot nest deeper with token '{token}'"
                )
            ...
            if isinstance(current, list):
                if token == "-":
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"invalid JSON pointer: list '{current_ptr}'"
                            f" require numbers as keys, instead got '{token}'"
                        ) from e
