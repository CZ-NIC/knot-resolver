...
class EscapedStringMin32(EscapedString, BaseStringLength):
    _min_bytes: int = 32
...
class IPAddressPort(BaseString):
    ...
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            if "@" not in self._value:
                msg = f"expected '<ip-address>@<port>', got '{self._value}'."
    ...
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            ...
            try:
                ...
                msg = f"expected IP address, got '{ip_address}'."
                raise DataValueError(msg, self._tree_path) from e


class IPv4Address(BaseString):
    def _validate(self, context: Context) -> None:
        super()._validate(context)
        if context.strictness > Strictness.PERMISSIVE:
            try:
                _ = ipaddress.IPv4Address(self._value)
            except ValueError as e:
                msg = f"expected IPv4 address, got '{self._value}'."
                raise DataValueError(msg, self._tree_path) from e


class IPv6Address(BaseString):
    def _validate(self, context: Context) -> None:
        super()._validate(context)
        if context.strictness > Strictness.PERMISSIVE:
            try:
                _ = ipaddress.IPv6Address(self._value)
            except ValueError as e:
                msg = f"expected IPv4 address, got '{self._value}'."
                raise DataValueError(msg, self._tree_path) from e
...
class IPAddressEM(BaseString):
    ...
    def _validate(self, context: Context) -> None:
        super()._validate(context)
        if context.strictness > Strictness.PERMISSIVE:
            if not self._value.endswith("!"):
                msg = f"expected string ending with '!', got '{self._value}'."
                raise DataValueError(msg, self._tree_path)
            try:
                ip_address = self._value[:-1]
                self.ip_address = ipaddress.ip_address(ip_address)
            except ValueError as e:
                msg = f"expected IP address, got '{ip_address}'."
                raise DataValueError(msg, self._tree_path) from e
    ...
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            ...
            try:
                ...
                try:
                    interface_name = InterfaceName(splited[0])
...
class IPNetwork(BaseString):
    ...
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            try:
                self.ip_network = ipaddress.ip_network(self._value)
            except ValueError as e:
                msg = f"expected IP network, got '{self._value}'."
                raise DataValueError(msg, self._tree_path) from e


class IPv6Network(BaseString):
    ip_network: ipaddress.IPv4Network | ipaddress.IPv6Network
    ...
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            try:
                self.ip_network = ipaddress.IPv6Network(self._value)
            except ValueError as e:
                msg = f"expected IPv6 network, got '{self._value}'."
                raise DataValueError(msg, self._tree_path) from e
...
class IPv6Network96(IPv6Network):
    def _validate(self, context: Context) -> None:
        ...
        if context.strictness > Strictness.PERMISSIVE:
            if self.ip_network.prefixlen == 128:
                msg = (
            ...
            if self.ip_network.prefixlen != 96:
                msg = (
                    "expected IPv6 network address with /96 prefix length,"
                    f" got prefix length of {self.ip_network.prefixlen}."
                )
                raise DataValueError(msg, self._tree_path)
