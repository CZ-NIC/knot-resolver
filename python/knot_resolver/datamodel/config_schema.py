...
from knot_resolver.datamodel.dga_filter_schema import DGAFilterSchema
...
def _get_views_tags(views: List[ViewSchema]) -> List[str]:
    ...
    for view in views:
        ...
        if view.tags_audit:
            tags += [str(tag) for tag in view.tags_audit if str(tag) not in tags]
    return tags


def _check_local_data_tags(
    views_tags: List[str], rules_or_rpz: Union[List[RuleSchema], List[RPZSchema], List[TunnelFilterSchema], List[DGAFilterSchema]]
...
class KresConfig(ConfigSchema):
    class Raw(ConfigSchema):
        ...
        dga_filter: Block suspected domains generated by DGA.
        ...
        dga_filter: DGAFilterSchema = DGAFilterSchema()
    ...
    def _validate(self) -> None:  # noqa: C901, PLR0912
        ...
        if self.dga_filter:
            dga_tags, dga_errs = _check_local_data_tags(views_tags, [self.dga_filter])
            errs += dga_errs
            local_data_tags += dga_tags
