...
class UncheckedPath(BaseValueType):
    ...
    def __init__(
        ...
        if isinstance(source_value, str):
            ...
            if self._parents:
                pp = (p.to_path() for p in self._parents)
...
def _check_permission(dest_path: Path, perm_mode: _PermissionMode) -> bool:
    ...
    dest_stat = os.stat(dest_path)  # noqa: PTH116
    ...
    for perm in _PermissionMode:  # noqa: SIM110
        if perm in perm_mode and not accessible(perm):
            return False
    return True
